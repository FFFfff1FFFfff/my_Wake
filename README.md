...
在没有声卡的云服务器上，可以使用 HTTP API 模式部署：

### 启动 API 服务器

```bash
python -m src.inference.api_server \
    --model models/wake_word_model.onnx \
    --host 0.0.0.0 \
    --port 8000
```
...
with open("audio.wav", "rb") as f:
    response = requests.post(
        "http://your-server:8000/detect_and_transcribe",
        files={"audio": f}
    )
    result = response.json()
    if result['detected']:
